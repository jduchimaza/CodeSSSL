...
public class TopClass implements ActionListener, KeyListener {
	...
	private void buildFrame() {
        ...
        f.setIconImage(icon);
        f.addKeyListener(this); // JD_ we want to respond to spacebar taps
	}
	...
	private JPanel createContentPane() {
		...
		startGame.setOpaque(true); // JD_ added line to make button blue
		startGame.setBorderPainted(false);// JD_ added line to make full button blue
	...
	// Keep this blank because we're simply implementing a KeyListener interface
	...
	private void gameScreen(boolean isSplash) {
		// At any given time, the most you have on screen are the bird, two top pipes, and two bottom pipes
	...
	private void collisionDetection(BottomPipe bp1, BottomPipe bp2, TopPipe tp1, TopPipe tp2, Bird bird) {
		...
		if(bird.getY() + BIRD_HEIGHT > SCREEN_HEIGHT*7/8) { //ground detection
			pgs.sendText("Game Over - Press B to play again");
			loopVar = false;
